...
namespace Tdlr.Utils
{
    class GraphHelper
    {
        ...
        public async static Task GetDirectoryObjects(List<string> objectIds, List<Group> groups, List<DirectoryRole> roles, List<User> users)
        {
            ...
            ActiveDirectoryClient graphClient = new ActiveDirectoryClient(new Uri(String.Format(CultureInfo.InvariantCulture, ConfigHelper.GraphServiceRoot, ClaimsPrincipal.Current.FindFirst(Globals.TenantIdClaimType).Value)), async () => { return GraphHelper.AcquireToken(userObjectId); });
